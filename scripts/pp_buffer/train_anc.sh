...
if [ "$#" -ne 6 ] ;then
  ...
  echo "Need 6 parameters for the GPUs, way, shot, data, strategy and hop"
  exit 1 
fi
gpus=$1
arch=simpleCnn
...
CUDA_VISIBLE_DEVICES=${gpus} python ./exp/train_pp_buffer.py \
	       --reset_interval ${reset_interval} \
	       --coef_base 0 --coef_anc 1 --coef_chi 0 --training_strategy ${training_strategy} \
               --arch ${arch} --att_arch ${att_arch} --n_hop ${n_hop} \
	       --dataset_root ${HOME}/datasets/tiered-imagenet-${data}/ --workers 16 \
	       --log_dir ./logs/${data}_${classes_per_it_val}way${num_support_val}shot_base0_anc1_chi0_ppbuffer_${training_strategy}_nhop${n_hop}_reset${reset_interval}/ --log_interval 20 --test_interval 100 \
